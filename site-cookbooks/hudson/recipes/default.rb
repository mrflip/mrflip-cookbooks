...
Chef::Log.info ['starting hudson recipe', __FILE__]
...
when "ubuntu", "debian"
  ...
  remote_file "#{tmp}/jenkins-ci.org.key" do
    ...
    source "http://pkg.jenkins-ci.org/debian/jenkins-ci.org.key" #  "#{node[:hudson][:mirror]}/debian/hudson-ci.org.key"
  end
...
when "centos", "redhat"
  ...
  remote = "#{node[:hudson][:mirror]}/latest/redhat/jenkins.rpm"
  package_provider = Chef::Provider::Package::Rpm
  pid_file = "/var/run/jenkins.pid"
  install_starts_service = false

  execute "add-jenkins-key" do
    command "rpm --import #{node[:hudson][:mirror]}/redhat/jenkins-ci.org.key"
    action :nothing
  end
...
ruby_block "netstat" do
  block do
    10.times do
      ...
      Chef::Log.debug("service[jenkins] still listening (port #{node[:hudson][:server][:port]})")
      sleep 1
    end
  end
  action :nothing
