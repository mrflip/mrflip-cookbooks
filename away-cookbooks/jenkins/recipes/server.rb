...
directory "#{node[:jenkins][:server][:home_dir]}/plugins" do
  owner     node[:jenkins][:server][:user]
  group     node[:jenkins][:server][:group]
...
node[:jenkins][:server][:plugins].each do |name|
  plugin_file = "#{node[:jenkins][:server][:home_dir]}/plugins/#{name}.hpi"
...
log "plugins updated, restarting jenkins" do
  ...
  only_if do
    if File.exists?(pid_file)
      ...
      Dir["#{node[:jenkins][:server][:home_dir]}/plugins/*.hpi"].select { |file|
